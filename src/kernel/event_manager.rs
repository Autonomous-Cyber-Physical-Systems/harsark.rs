    ...
    pub fn sweep(&mut self, event_type: EventTableType) {
        match event_type {
            EventTableType::MilliSec => {
                self.ms_event_table.table.clone().iter().for_each(|event_id| {
                    self.execute_event(*event_id);
                });
            },
            EventTableType::Sec => {
                self.sec_event_table.table.clone().iter().for_each(|event_id| {
                    self.execute_event(*event_id);
                });
            },
            EventTableType::Min => {
                self.min_event_table.table.clone().iter().for_each(|event_id| {
                    self.execute_event(*event_id);
                });
            },
            EventTableType::Hour => {
                self.hr_event_table.table.clone().iter().for_each(|event_id| {
                    self.execute_event(*event_id);
                });
            },
        }
